...
struct SAPI WindowConfig {
    u32 flags;
    i32 antialiasingLevel;
};

SAPI bool8 InitWindow(const char* title, i32 width, i32 height, WindowConfig config);
...
SAPI bool8 IsKeyPressed(KeyboardKey key);
SAPI bool8 IsKeyReleased(KeyboardKey key);
SAPI bool8 IsKeyDown(KeyboardKey key);
SAPI bool8 IsKeyUp(KeyboardKey key);

SAPI Vec2 GetMousePosition();
SAPI Vec2 GetMouseWheel();
SAPI bool8 IsMousePressed(MouseButton button);
